...
| Cost abuse | Denial of wallet | Excessive or concurrent provider requests, stale/missing price configuration | Margin loss | Per-session debounce/cap and serialization, conservative token/output ceilings, RLS-isolated idempotent usage ledger, worst-case priced admission, fail-closed accounting outage; reviewed provider prices remain an operational prerequisite |
