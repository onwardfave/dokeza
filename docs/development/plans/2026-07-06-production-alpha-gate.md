...
5. Add desktop sign-in, sign-out, workspace selection, token refresh/retry states, and secure token storage. Secure API session token storage and hosted PKCE/loopback token-exchange foundations are done; hosted IdP refresh/session renewal remains open.
